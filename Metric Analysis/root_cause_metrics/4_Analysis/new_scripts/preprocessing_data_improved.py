...
discard_first_n_samples_test = 0
window_length = 10

service = "carts"
experiment = "normal"
other_service = "payment"
path = "/home/matilda/PycharmProjects/RCA_metrics /3_Preprocessing_data/" + experiment + "/" + service + ".csv"

store_path_normal_train = "/home/matilda/PycharmProjects/RCA_metrics /3_Preprocessing_data/ready_imrpovedMicroRCA/" + experiment + "/" + service + "_" +other_service+ "_train.csv"
store_path_normal_validation = "/home/matilda/PycharmProjects/RCA_metrics /3_Preprocessing_data/ready_imrpovedMicroRCA/" + experiment + "/" + service + "_" + other_service + "_validation.csv"
...
path = "/home/matilda/PycharmProjects/RCA_metrics /3_Preprocessing_data/improve_MicroRCA/" + experiment + "/" + service +"/" + other_service + ".csv"

store_path_test_descritpion = "/home/matilda/PycharmProjects/RCA_metrics /3_Preprocessing_data/ready_imrpovedMicroRCA/" + experiment  + "/" + service + "/" + other_service +  "_test.csv"
store_path_test_labels = "/home/matilda/PycharmProjects/RCA_metrics /3_Preprocessing_data/ready_imrpovedMicroRCA/" + experiment + "/" + service + "/" + other_service +  "_labels.csv"
...
abnormal_data, indecies = dataset_abnormal.data.iloc[:, 1:], False
targets = copy.copy(dataset_abnormal.data)
targets["target"] = np.zeros(targets.shape[0], dtype="int")
targets = targets.loc[:, ["timestamp", "target"]]
...
new = []
for x in preserve_col:
    if service in x:
        print(x)
        x = x.replace(service, other_service)
    new.append(x)
preserve_col = new
...
abnormal_data = abnormal_data.iloc[discard_first_n_samples_test:, :]
...
for x in range(60, abnormal_data.shape[0]-60):

    if targets.target.values[x+60] == 1 and targets.target.values[x-60] == 0:
        anom_indecies[key].append(x)
    elif targets.target.values[x-60] == 0:
        normal_indecies.append(x)

    if targets.target.values[x] == 1 and controlVar==False:
        controlVar = True

    if targets.target.values[x] == 0 and controlVar:
        controlVar = False
        key +=1
...
p1 = pd.DataFrame(abnormal_data).iloc[normal_indecies, :]

ratio = 0.99
train_ind = int(len(normal_indecies)*ratio)





sc, abnormal_data_train = minmaxscale_train(p1.values)

p1 = pd.DataFrame(abnormal_data_train)
p1.columns = preserve_col

train_path = "/home/matilda/PycharmProjects/RCA_metrics /3_Preprocessing_data/ready_imrpovedMicroRCA/" + experiment + "/" + service +"/" + other_service + "_train.csv"
valid_path = "/home/matilda/PycharmProjects/RCA_metrics /3_Preprocessing_data/ready_imrpovedMicroRCA/" + experiment + "/" + service +"/" + other_service + "_valid.csv"
p1.iloc[:train_ind, :].to_csv(train_path, index=False)
p1.iloc[train_ind:, :].to_csv(valid_path, index=False)

for key in anom_indecies.keys():
    store_path_test_descritpion = "/home/matilda/PycharmProjects/RCA_metrics /3_Preprocessing_data/ready_imrpovedMicroRCA/" + experiment  + "/" + service + "/" + other_service + "_test_" + str(key-1) + ".csv"
    store_path_test_labels = "/home/matilda/PycharmProjects/RCA_metrics /3_Preprocessing_data/ready_imrpovedMicroRCA/" + experiment + "/" + service + "/" + other_service + "_labels_" + str(key-1) + ".csv"
    pd.DataFrame(minmaxscale_test(abnormal_data.loc[anom_indecies[key]], sc)).to_csv(store_path_test_descritpion, index=False)
    pd.DataFrame(targets.target[anom_indecies[key]]).to_csv(store_path_test_labels, index=False)
