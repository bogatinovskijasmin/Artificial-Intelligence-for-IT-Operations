...
class LogParser:
    ...
    def parse(self, logName, batch_size=5, mask_percentage=1.0, pad_len=150, N=1, d_model=256,
        ...
        print(self.tokenizer.word2index.keys())
        ...
        for i, (x, x1, y, ind) in enumerate(results):
            print(x1)
            # print(ind)
            for j in range(len(x)):
                #if not self.num_there(self.tokenizer.index2word[y[j]]):
                    if y[j] in x[j][-self.k:] and np.std(x1[j][-self.k:]) > 0.5 :
    ...
    def run_test(self, dataloader, model, loss_compute):
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                #print(np.sort(out_p.cpu().numpy(),axis=1))
                yield out_p.cpu().numpy().argsort(axis=1), np.sort(out_p.cpu().numpy(),axis=1),b_labels.cpu().numpy(), ind.cpu()
